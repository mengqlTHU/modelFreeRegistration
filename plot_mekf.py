...
for i in range(pose_data.shape[0]):
    qt_estimate[i, :] = quaternion_mul_num(ekf_log[i, 11:15], ekf_log[i, 15:19])
...
for i in range(4):
    ...
    plt.plot(timestamp_data[1:-1,1], ekf_log[1:-1, 11+i], linewidth=3, color=color_list[i], label=f"$q_{{b}}{{}}_{i}$")
plt.xlabel('Time(s)')
...
for i in range(4):
    ...
    ax.plot(ekf_log[1:-1, 15+i], linewidth=3, label=f"$q_{{d}}{{}}_{i}$", color=color_list[i])
plt.xlabel('Iteration')
